...
final class PostType extends \tiFy\App\Factory
{
    ...
    protected $tFyAppActions                = array(
        'post_action_tiFyMultilingualDuplicatePost',
        'admin_notices'
    );
    ...
    final public function post_action_tiFyMultilingualDuplicatePost( $post_id )
    {
        // Bypass
        if( empty( $post_id ) )
            return new \WP_Error( __( 'Le contenu original est indisponible', 'tify' ), 'tiFyMultilingualDuplicatePost-UnavailableInput' );

        if( empty( $_REQUEST['lang'] ) )
            return new \WP_Error( __( 'La langue de traduction est indisponible', 'tify' ), 'tiFyMultilingualDuplicatePost-UnavailableLang' );
        ...
        wp_redirect( add_query_arg( array( 'message' => 'tiFyMultilingualDuplicatedPost' ), $sendback ) );
	    exit();
    }
    ...
    {                    
        ...
        if( ! Multilingual::hasTranslatePostType( $post_type ) ) :
            return new \WP_Error( __( 'Le type du contenu original n\'est pas autorisé à être dupliqué', 'tify' ), 'tiFyMultilingualDuplicatePost-InputTypeNotAllowed' );
        endif;
