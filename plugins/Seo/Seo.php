...
class Seo extends Plugin
{
	...
	protected $tFyAppActionsPriority	= array(
		'init' 	=> 20 // Avant tify_taboox_register_node (25)
	);	
	...
	public function __construct()
	{
		...
		add_filter( 'wp_title', array( $this, 'wp_title' ), 20, 3 );
					
		
		// Actions et Filtres tiFy SEO Plugin
		add_action( 'tify_seo_wp_head', array( $this, 'meta_description' ), 10 );
		add_filter( 'tify_seo_title_is_page_on_front', array( $this, 'tify_seo_title_for_singular' ), 0, 3 );		
		add_filter( 'tify_seo_title_is_single', array( $this, 'tify_seo_title_for_singular' ), 0, 3 );
		add_filter( 'tify_seo_title_is_page', array( $this, 'tify_seo_title_for_singular' ), 0, 3 );		
		add_filter( 'tify_seo_title_is_page_for_posts', array( $this, 'tify_seo_title_for_archive' ), 0, 3 );
		add_filter( 'tify_seo_desc_is_page_on_front', array( $this, 'tify_seo_desc_for_singular' ), 0, 3 );
		add_filter( 'tify_seo_desc_is_single', array( $this, 'tify_seo_desc_for_singular' ), 0, 3 );
		add_filter( 'tify_seo_desc_is_page', array( $this, 'tify_seo_desc_for_singular' ), 0, 3 );	
	}
	...
	final public function init()
	{
		...
		self::$post_type 		= array_diff( $post_type, array( 'attachment', 'revision', 'nav_menu_item' ) );
	...
	final public function wp_title( $title, $sep, $seplocation )
	{
		...
		elseif ( is_archive() )	:	
			if( is_post_type_archive() ) :
				$title = apply_filters( 'tify_seo_title_is_post_type_archive', post_type_archive_title( '', false ), $sep, $seplocation );
			else:
		...
		return $title . ( self::$append_blogname ? self::$sep . get_bloginfo( 'name' ) : '' ) . ( self::$append_sitedesc ? self::$sep . get_bloginfo( 'description', 'display' ) : '' );
	}
		...
		foreach( (array) self::$post_type as $post_type ) :
			...
			tify_taboox_register_node( 
				...
				array( 
					...
					'cb' 		=> "\\tiFy\\Plugins\\Seo\\Taboox\\Post\\MetaTag\\Admin\\MetaTag" 
				) 
			);
		endforeach;
	}
	...
	final public function tify_options_register_node(){	
		...
		tify_options_register_node(
			array(
				...
				'cb' 		=> "\\tiFy\\Plugins\\Seo\\Taboox\\Option\\OpenGraph\\Admin\\OpenGraph",
				'order'		=> 1
			)
		);
		tify_options_register_node(
			array(
				...
				'cb' 		=> "\\tiFy\\Plugins\\Seo\\Taboox\\Option\\GoogleAnalytics\\Admin\\GoogleAnalytics",
				'order'		=> 2
			)
		);
	}
	...
	final public function tify_seo_title_for_archive( $title )
	{
		...
		if( is_home() && ( $page_for_posts = get_option( 'page_for_posts' ) ) && ( $seo_meta = get_post_meta( $page_for_posts, '_tify_seo_meta', true ) ) && ! empty( $seo_meta['title'] ) )
			return $seo_meta['title'];
	...
	final public function tify_seo_desc_for_archive( $desc )
	{
		...
		if( is_home() && ( $page_for_posts = get_option( 'page_for_posts' ) ) && ( $seo_meta = get_post_meta( $page_for_posts, '_tify_seo_meta', true ) ) && ! empty( $seo_meta['desc'] ) )
			return $seo_meta['desc'];
