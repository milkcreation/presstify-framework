...
class tinymceDashicons{	
	...
	public function __construct(){
		...
		/// ACTION AJAX
		add_action( 'wp_ajax_tinymce-dashicons-class', array( $this, 'wp_ajax_action' ) );		
	}
	...
	public function wp_ajax_action(){
		...
		echo '.dashicons{ font-family: '. $this->options['font-family'] .'; }'; exit;
	}
	
	/* = CONTROLEUR = */
	/** == Ajout des styles dans l'éditeur == **/
 	public function add_tinymce_editor_style( $mce_css ) {
        $mce_css .= ', '. $this->options['css'] .', '. $this->uri.'/editor.css, '. admin_url( 'admin-ajax.php?action=tinymce-dashicons-class&bogus='.current_time( 'timestamp' ) );
