...
class Media
{
     ...
     * @var mixed
     */
    protected $AttachmentMetadata   = array();
    ...
    public function reset()
    {
        ...
        $this->UploadDir = array();
        $this->AttachmentMetadata = array();
        $this->UniqueFilename = '';
        
        // Réinitialisation des filtres de surcharge Wordpress
        remove_filter('upload_dir', array($this, 'upload_dir'), 10);
        remove_filter('wp_generate_attachment_metadata', array($this, 'wp_generate_attachment_metadata'), 10);
        remove_filter('wp_unique_filename', array($this, 'wp_unique_filename'), 10);
    }    
     ...
     * @return \WP_Error|int
     */
    public static function import($file, $attrs = array())
    {
        // Instanciation
        $Media = new static;
        ...
        if (! isset($upload_dir['error'])) :
            ...
            // Rapport d'erreur
            return new \WP_Error( 
                'tiFy\Statics\Medias::import|UploadDirInvalid', 
        ...
        elseif ($upload_dir['error'] !== false) :
            ...
            // Rapport d'erreur
            return new \WP_Error( 
                'tiFy\Statics\Medias::import|UploadDirError', 
                $upload_dir['error']
            );
        endif;    
        ...
        if ($is_url) :
            ...
            if (! $response = wp_remote_get($file)) :
                ...
                // Rapport d'erreur
                return new \WP_Error( 
                    'tiFy\Statics\Medias::import|RemoteGetFailed', 
            ...
            if ($code != '200') :
                ...
                // Rapport d'erreur
                return new \WP_Error(
                    'tiFy\Statics\Medias::import|RemoteCode200', 
                    sprintf( 
        ...
        // Le fichier n'existe pas
            ...
            // Rapport d'erreur
            return new \WP_Error(
                'tiFy\Statics\Medias::import|FileNotExist', 
        ...
        if ($attrs['upload_dir']) :
            ...
            add_filter('wp_generate_attachment_metadata', array($Media, 'wp_generate_attachment_metadata'), 10, 2);
        endif;
        
        // Traitement de l'écrasement de fichier
        if ($attrs['override']) :
            $Media->UniqueFilename = $name;
            add_filter('wp_unique_filename', array($Media, 'wp_unique_filename'), 10, 4);
        endif;
        ...
        if (! empty($upload['error'])) :
            ...
            // Rapport d'erreur
            return new \WP_Error(
                'tiFy\Statics\Medias::import|UnableUploadBits', 
                $upload['error']
            );
        endif;        
        ...
        if (0 == $filesize) :
            ...
            // Rapport d'erreur
            return new \WP_Error( 
                'tiFy\Statics\Medias::import|EmptyUploadFile', 
     ...
     * @return \WP_Error|int
     */
    public static function addAttachedFile($post_parent, $file, $attrs = array())
    {
        // Contrôle d'intégrité du répertoire de stockage
        ...
        if (! $finfo['ext'] || ! $finfo['type']) :
            return new \WP_Error(
                'tiFy\Statics\Media::add|UnavailableMimeType',
        ...
        if(! $filemtime = filemtime($file)) :
            return new \WP_Error(
                'tiFy\Statics\Media::add|UnavailableFileMtime',
        ...
        foreach ($exists as $exist_id) :
            ...
            // Vérification de présence du fichier existant
            ...
            elseif (dirname($exist_path) !== $upload_dir['path']):
     ...
     * }
     */
    public static function uploadDir($base = null, $sub = null)
    {
        if (is_null($base) && is_null($sub))
            return wp_upload_dir();
        
        if ($base) :
            $rel = File::getRelativeFilename($base);
            if (! is_null($rel)):
            ...
            /*elseif (preg_match('#^'. preg_quote($base, '/') .'(.*)#', ABSPATH, $matches)) :
                ...
                endforeach;
                $basedir = $base;
                $baseurl = site_url('/' . $forward_url);*/
            else :
                return array(
                    'path' => $base . $sub, 'url' => '', 'subdir' => $sub, 'basedir' => $base, 'baseurl' => '', 
                    'error' => sprintf(
                        __('Impossible de joindre le répertoire %s', 'tify'),
                        $base . $sub
                    )
                );
            endif;
		...
		if (! wp_mkdir_p($path)) :
			...
			$error = sprintf(
				__('Impossible de créer le répertoire %s', 'tify'),
				esc_html($error_path)
			);
		endif;  
