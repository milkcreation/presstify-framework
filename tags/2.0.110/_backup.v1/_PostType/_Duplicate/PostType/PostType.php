...
final class PostType extends \tiFy\App\Factory
{
    ...
    public function __construct()
    {
        ...
        $this->appAddAction('post_action_tiFyDuplicatePost');
        $this->appAddAction('admin_notices');
        $this->appAddFilter('admin_enqueue_scripts');
        $this->appAddFilter('page_row_actions', 'row_actions', 99, 2);
        $this->appAddFilter('post_row_actions', 'row_actions', 99, 2);
    }
    ...
    final public function post_action_tiFyDuplicatePost($post_id)
    {
        ...
        if (empty($post_id)) {
            return new \WP_Error(__('Le contenu original est indisponible', 'tify'),
                'tiFyDuplicatePost-UnavailableInput');
        }
        ...
        wp_redirect(add_query_arg(['message' => 'tiFyDuplicatedPost'], $sendback));
        exit();
    }
    ...
    final public function row_actions($actions, $post)
    {
        ...
        $actions['tiFyDuplicatePost'] = "<a href=\"" . wp_nonce_url(add_query_arg(['post'   => $post->ID,
                                                                                   'action' => 'tiFyDuplicatePost',
            ], admin_url(sprintf($post_type_object->_edit_link, $post->ID))),
                'tify_duplicate_post:' . $post->ID) . "\" title=\"" . __('Dupliquer le contenu',
                'tify') . "\" class=\"tiFyDuplicatePost-rowAction\">" . __('Dupliquer', 'tify') . "</a>";
    ...
    private function duplicate($post_id)
    {
        ...
        if (!$post_type_attrs = Duplicate::getPostType($post_type)) {
            return new \WP_Error(__('Le type du contenu original n\'est pas autorisé à être dupliqué', 'tify'),
                'tiFyDuplicatePost-InputTypeNotAllowed');
        }
