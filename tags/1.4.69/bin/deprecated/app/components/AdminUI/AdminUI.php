...
class AdminUI extends Component
{
    ...
    /**
     * Après le chargement du thème Wordpress.
     *
     * @return void
     */
    ...
    {
        if (!$admin_bar_menu_logo = $this->appConfig('admin_bar_menu_logo')) :
            return;
        endif;
    ...
    {
        if ($admin_footer_text = $this->appConfig('admin_footer_text')) :
    ...
    {
        foreach ((array)$this->appConfig('remove_menu') as $menu) :
            switch ($menu) :
                default :
    ...
    private function remove_support_post_type()
    {
        foreach (array_keys($this->appConfig()) as $config) :
            ...
            foreach ((array)$this->appConfig($config) as $support) {
    ...
    {
        foreach ((array)$this->appConfig('unregister_widget') as $widget) :
            switch ($widget) :
                default :
    ...
    {
        foreach ((array)$this->appConfig('remove_dashboard_meta_box') as $metabox => $context) :
    ...
    private function remove_meta_box_post_types()
    {
        foreach (array_keys($this->appConfig()) as $config) :
            ...
            foreach ((array)$this->appConfig($config) as $metabox => $context) :
                if (is_numeric($metabox)) :
                    $_metabox = $context;
    ...
    private function remove_admin_bar_menu()
    {
        ...
        foreach ((array)$this->appConfig('remove_admin_bar_menu') as $admin_bar_node) :
    ...
    final public function unregister_taxonomy_for_object_type()
    {
        ...
        foreach (array_keys($this->appConfig()) as $config) :
            ...
            if (!$taxonomies = $this->appConfig('unregister_taxonomy_for_' . $post_type[1])) {
                continue;
            }
