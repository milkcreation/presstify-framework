...
class PdfViewer extends \tiFy\App\Component
{
     ...
     * @var array'wp_ajax'
     */
    private static $defaultToggleAttrs = [
        'id'    => '',
        'class' => '',
        'href'  => '',
        'text'  => '',
     ...
     * @var array
     */
    private static $defaultModalAttrs = [
        'id'      => '',
        'target'  => '',
    ...
    public function __construct()
    {
        ...
        $this->appAddAction('wp_ajax_tify_components_pdfviewer_modal', 'wp_ajax');
        $this->appAddAction('wp_ajax_nopriv_tify_components_pdfviewer_modal', 'wp_ajax');
    }
    ...
    final public function init()
    {
        ...
        wp_register_script('pdf-js', '//cdnjs.cloudflare.com/ajax/libs/pdf.js/1.7.225/pdf.min.js', [], '1.7.225', true);
        wp_register_style('tiFyComponentsPdfViewer', self::tFyAppUrl() . '/PdfViewer.css', ['dashicons'], '170321');
        wp_register_script('tiFyComponentsPdfViewer', self::tFyAppUrl() . '/PdfViewer.js',
            ['jquery', 'jquery-ui-widget', 'pdf-js'], '170321', true);
        wp_localize_script('tiFyComponentsPdfViewer', 'tiFyComponentsPdfViewer', ['workerSrc' => $worker_src]);
        /**
         * Modal
         */
        wp_register_style('tiFyComponentsPdfViewerModal', self::tFyAppUrl() . '/Modal.css', ['tiFyComponentsPdfViewer'],
            '170615');
        wp_register_script('tiFyComponentsPdfViewerModal', self::tFyAppUrl() . '/Modal.js', ['tiFyComponentsPdfViewer'],
            '170615', true);
    }
     ...
     * @return string|array
     */
    private static function parsePdfArgs($args = [], $footer_buttons = false)
    {
     ...
     * @return string|array
     */
    private static function parseToggleAttrs($args = [])
    {
     ...
     * @param $footer_buttons Affichage des organes de navigation dans le footer de la modal
     */
    private static function parseModalAttrs($pdf_url = null, $args = [], $footer_buttons = false)
    {
    ...
    protected static function getPdfNavButton($type = '')
    {
        switch ($type) :
            case 'prev' :
                return "<button type=\"button\" class=\"tiFyPdfViewer-nav tiFyPdfViewer-nav--prev\">" . __('Précédent',
                        'tify') . "</button>";
                break;
            case 'next' :
                return "<button type=\"button\" class=\"tiFyPdfViewer-nav tiFyPdfViewer-nav--next\">" . __('Suivant',
                        'tify') . "</button>";
                break;
            case 'download' :
                return "<button type=\"button\" class=\"tiFyPdfViewer-download\">" . __('Télécharger',
                        'tify') . "</button>";
                break;
            default :
                return null;
     ...
     * @return void|string
     */
    public static function display($pdf_url = null, $args = [], $echo = true)
    {
        // Bypass
        if (!$pdf_url) {
            return;
     ...
     * @return void|string
     */
    public static function modalToggle($pdf_url = null, $args = [], $footer_buttons = true, $echo = true)
    {
        // Bypass
        if (!$pdf_url) {
            return;
