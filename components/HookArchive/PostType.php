...
final class PostType extends Factory
{
    ...
    public function __construct($args = [])
    {
        ...
        add_filter('post_type_archive_link', [$this, 'post_type_archive_link'], 99, 2);

        add_filter('tify_breadcrumb_is_single', [$this, 'tify_breadcrumb_is_single'], 10, 2);
        add_filter('tify_breadcrumb_is_archive', [$this, 'tify_breadcrumb_is_archive']);
        add_filter('tify_seo_title_is_post_type_archive', [$this, 'tify_seo_title_is_post_type_archive']);
        add_filter('tify_seo_desc_is_post_type_archive', [$this, 'tify_seo_desc_is_post_type_archive']);
    }
    ...
    final public function registered_post_type($post_type, $args)
    {
        ...
        foreach ((array)$this->GetHooks() as $hook) :
            ...
            add_rewrite_rule("{$archive_slug}/?$", "index.php?post_type={$post_type}&tify_hook_id={$hook['id']}",
                'top');
            add_rewrite_rule("{$archive_slug}/{$wp_rewrite->pagination_base}/([0-9]{1,})/?$",
                "index.php?post_type={$post_type}&tify_hook_id={$hook['id']}" . '&paged=$matches[1]', 'top');
            ...
            if ($this->Options['rewrite'] && $hook['permalink']) :
                if ($args->hierarchical) {
                    add_rewrite_tag(
                        "%$post_type%",
                        '(.+?)',
                        $args->query_var ? "{$args->query_var}=" : "post_type=$post_type&pagename=");
                } else {
                    add_rewrite_tag(
                        "%$post_type%",
                        '([^/]+)',
            ...
            remove_action('registered_post_type', [$this, 'registered_post_type']);
        endforeach;
    }
    ...
    final public function display_post_states($post_states, $post)
    {
        ...
        $post_states['hookarchive_for_' . get_post_type($post)] = sprintf(__('Page des %s', 'tify'),
            strtolower($label));
            ...
            if (!empty($hook_id) && ($post = get_post($hook_id))) :
                ...
                if ($post->post_parent && $post->ancestors) :
                    ...
                    foreach ($parents as $parent) :
                        $ancestors[] = ['url'   => get_permalink($parent),
                                        'name'  => $Template::titleRender($parent),
                                        'title' => $Template::titleRender($parent)
                        ];
                    endforeach;
                endif;
                $ancestors[] = ['url'   => get_post_type_archive_link(get_post_type()),
                                'name'  => $Template::titleRender($hook_id),
                                'title' => $Template::titleRender($hook_id)
                ];
                ...
                $part = ['name' => $Template::titleRender(get_the_ID())];
