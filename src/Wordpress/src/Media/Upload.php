...
class Upload implements UploadContract
{
    ...
    /**
     * Instance du repertoire de desination.
    ...
    public function add(string $file, array $args = []): int
    {
        ...
        if (!$finfo['ext'] || !$finfo['type']) {
            ...
            throw new Exception(sprintf(
                'La taille du fichier dépasse le poids maximum de %s autorisé.', size_format($maxSize)
            ));
        }
        ...
        foreach ($exists as $id) {
            ...
            if ($this->storage()->path('/') . $name  === $path) {
                $args['attachment_id'] = $id;
                break;
            }
        }

        if ($args['attachment_id'] && !$this->isRenewable() && $this->storage()->has($name) && ($meta = wp_get_attachment_metadata($args['attachment_id']))) {
        ...
        try {
            $this->storage()->write($name, file_get_contents($file));
        } catch (Exception $e) {
        ...
        $id = wp_insert_attachment([
            ...
            'post_mime_type' => $this->storage()->getMimetype($name),
        ...
        if ($id instanceof WP_Error) {
            ...
            wp_update_attachment_metadata($id, array_merge(
                wp_generate_attachment_metadata($id, $file), compact('filemtime', 'filesize')
            ));
        }
    ...
    public function storage(): ?LocalFilesystem
    {
        if (is_null($this->storage)) {
            ...
            $this->storage = (!is_dir($dir) && !wp_mkdir_p($dir)) ? null : Storage::local($dir);
        }
    ...
    public function params($key = null, $default = null)
    {
        if (!$this->params instanceof ParamsBag) {
            $this->params = (new ParamsBag())->set($this->defaultParams());
        }
     ...
     * @inheritDoc
     */
    public function setMaxSize(int $size): UploadContract
    {
     ...
     * @inheritDoc
     */
    public function setRenewable(bool $renew = true): UploadContract
    {
     ...
     * @inheritDoc
     */
    public function setParams(array $params): UploadContract
    {
        $this->params($params);
     ...
     * @inheritDoc
     */
    public function setStorageDir(string $dir): UploadContract
    {
