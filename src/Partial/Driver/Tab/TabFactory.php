...
use tiFy\Contracts\Partial\TabCollection;
use tiFy\Contracts\Partial\TabFactory as TabFactoryContract;
use tiFy\Support\HtmlAttrs;
use tiFy\Support\ParamsBag;

class TabFactory extends ParamsBag implements TabFactoryContract
{
    /**
     ...
     * @inheritDoc
     */
    public function boot(): TabFactoryContract
    {
        if (!$this->booted) {
            $this->parse();
     ...
     * @inheritDoc
     */
    public function build(): TabFactoryContract
    {
        if (!$this->built) {
            try {
                $this->generateIds();
     ...
     * @inheritDoc
     */
    public function getParent(): ?TabFactoryContract
    {
    ...
    {
        return ($parent = $this->getParent()) instanceof TabFactoryContract ? $parent->getName() : '';
    }
     ...
     * @inheritDoc
     */
    public function setCollection(TabCollection $collection): TabFactoryContract
    {
     ...
     * @inheritDoc
     */
    public function setDepth(int $depth = 0): TabFactoryContract
    {
        $this->depth = $depth;
