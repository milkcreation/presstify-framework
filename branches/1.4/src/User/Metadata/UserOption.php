...
final class UserOption extends AppController
{
    /* = ARGUMENTS = */
    ...
    {
        add_action('profile_update', [$this, 'Save'], 10);
        add_action('user_register', [$this, 'Save'], 10);
    }
    ...
    final public static function Register($meta_key, $single = false, $sanitize_callback = 'wp_unslash')
    {
        ...
        if ($sanitize_callback !== '') :
            add_filter("tify_sanitize_option_user_{$meta_key}", $sanitize_callback);
        endif;
    }
    ...
    final public static function Get($user_id, $meta_key)
    {
        ...
        if ($order = get_user_option('_order_' . $meta_key, $user_id)) {
            $query .= " ORDER BY FIELD( {$wpdb->usermeta}.user_id," . implode(',', $order) . ")";
        }

        if (!$metas = $wpdb->get_results($wpdb->prepare($query, $user_id, $wpdb->get_blog_prefix() . $meta_key))) {
            return;
        }
    ...
    final public function Save($user_id)
    {
        ...
        // Vérification d'existance d'options utilisateurs déclarées	
        if (empty(self::$MetaKeys)) {
            return;
        }

        // Récupération des metadonnés en $_POST
        $request = (isset($_POST['tify_option_user'])) ? $_POST['tify_option_user'] : null;
        ...
        foreach ((array)$meta_keys as $meta_key) :
            ...
            foreach ((array)$usermeta[$meta_key] as $meta_id => $meta_value) :
                $meta_value = apply_filters("tify_sanitize_option_user_{$meta_key}", $meta_value);

                if (is_int($meta_id) && get_metadata_by_mid('user', $meta_id)) :
                    $_meta_id = $meta_id;
                    update_metadata_by_mid('user', $meta_id, $meta_value);
                else :
                    $_meta_id = update_user_option($user_id, $meta_key, $meta_value);
                endif;
            ...
            if (!empty($order)) {
                update_user_option($user_id, '_order_' . $meta_key, $order);
            }
        endforeach;
