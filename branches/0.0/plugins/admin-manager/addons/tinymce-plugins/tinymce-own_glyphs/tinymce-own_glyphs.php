...
class tinymceOwnGlyphs{	
	...
	public function __construct(){
		...
		/// ACTION AJAX
		add_action( 'wp_ajax_tinymce-own_glyphs-class', array( $this, 'wp_ajax_action' ) );
	}
	...
	public function wp_init(){
		...
		$this->options = apply_filters( 'tify_tinymce_own_glyphs_options', array( 
		...
		wp_register_style( 'tinymce-own_glyphs', $this->uri .'/plugin.css', array(), '20141219' );
	...
	public function wp_admin_enqueue_scripts() {
		...
		wp_enqueue_style( 'tinymce-own_glyphs' );
    }
	...
	?>
	<style type="text/css">.own_glyphs{ font-family: '<?php echo $this->options['font-family'];?>'; font-style:normal; }</style>
	<?php
	}
	...
	public function wp_ajax_action(){
		...
		echo '.own_glyphs{ font-family: '. $this->options['font-family'] .'; }'; exit;
	}
	
	/* = CONTROLEUR = */
	/** == Ajout des styles dans l'éditeur == **/
 	public function add_tinymce_editor_style( $mce_css ) {
        $mce_css .= ', '. $this->options['css'] .', '. $this->uri.'/editor.css, '. admin_url( 'admin-ajax.php?action=tinymce-own_glyphs-class&bogus='.current_time( 'timestamp' ) );
