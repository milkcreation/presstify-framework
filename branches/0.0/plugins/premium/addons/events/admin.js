jQuery( document ).ready( function($){
	...
	$( document ).on( 'tify_touch_time_change', function( e ){
		...
		if( $closest.hasClass( 'tify_event_start_datetime-wrapper' ) ){
			...
			if( ! ( end >= start ) ){
				$( '.tify_control_touch_time-input', $parent.find( '.tify_event_end_datetime-wrapper' ) ).val( $( '.tify_control_touch_time-input', $closest ).val() );
				touchTimeInput2Handler( $( '.tify_control_touch_time-input', $parent.find( '.tify_event_end_datetime-wrapper' ) ) );
	...
	// Bascule d'affichage de la date de fin
	$( document ).on( 'change', '.tify_events-taboox .tify_event_end_datetime-toggle', function(e){
		$(this).closest( 'th' ).next().find( '> div').fadeToggle();		
	});
	$( '.tify_events-taboox .tify_event_end_datetime-toggle' ).each( function(){
		if( $(this).is( ':checked' ) )
			$(this).closest( 'th' ).next().find( '> div' ).show();
		else
			$(this).closest( 'th' ).next().find( '> div' ).hide();
	});
