...
class tiFy_SocialShare_Instagram{
	...
	function wp_after_setup_theme(){
		if( ! $this->master->actives['instagram'] )
			return;	
	...
	function tify_taboox_register_node(){
		tify_options_register_node(	
			array(
				'id' 		=> 'site-options-sn-instagram',
				'parent' 	=> 'tify_social_share',
				'title' 	=> __( 'Instagram', 'tify' ),
		...
		<table class="form-table">
			<tbody>			
				<tr>
					...
					<td>
						<input type="text" name="<?php echo $this->name;?>[instagram][uri]" value="<?php echo $this->value['instagram']['uri'];?>" size="80" placeholder="<?php _e( 'https://instagram.com/[nom du compte]', 'tify' );?>" />
					</td>
				</tr>
			</tbody>
		</table>
