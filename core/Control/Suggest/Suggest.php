...
class Suggest extends \tiFy\Core\Control\Factory
{
    ...
    {
        wp_register_style( 'tify_control-suggest', self::tFyAppAssetsUrl('Suggest.css', get_class()), array('tiFyTheme' ), '160222' );
        wp_register_script( 'tify_control-suggest', self::tFyAppAssetsUrl('Suggest.js', get_class()), array('jquery-ui-autocomplete' ), '160222', true );
        wp_localize_script( 
        ...
        add_action( 'wp_ajax_tify_control_suggest_term', array( $this, 'ajax' ) );
        add_action( 'wp_ajax_nopriv_tify_control_suggest_term', array( $this, 'ajax' ) );
    }
    ...
    public static function display( $attrs = array(), $echo = true )
    {
        ...
        $output .= ">\n";
        $output .= $before;
        $output .= "\t<input type=\"text\" class=\"tiFyControlSuggest-textInput\" placeholder=\"{$placeholder}\" autocomplete=\"off\"". ( $readonly ? ' readonly' : '' ) ." value=\"". ( ( $select && !is_bool($select) ) ? $select : '' ) ."\">\n";
        ...
        while( $query_post->have_posts() ) : $query_post->the_post();
            ...
            foreach( $elements as $el ) :
                if( method_exists( $this, 'item_value_'. $el ) ) :
                    ${$el} = call_user_func( array( $this, 'item_value_'. $el ) );
                else :
                    ${$el} = null;
                endif;
            endforeach;
            ...
            // Génération du rendu
            $render = call_user_func( array( $this, 'itemRender' ), compact( $elements ) );
     ...
     */
    final public function itemRender( $attrs = array() )
    {
        $output  = "";
        foreach( $attrs as $attr => $value ) :
            if( method_exists( $this, 'item_render_'. $attr ) ) :
                $output .= call_user_func( array( $this, 'item_render_'. $attr ), $value, $attrs );
            else :
    ...
    {
        return get_the_post_thumbnail( null, array(50,50), false );
    }
